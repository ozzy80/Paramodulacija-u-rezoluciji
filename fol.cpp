...
void VariableTerm::getVars(VariableSet &vars) const { vars.insert(_v); }
...
void LogicConstant::getVars(VariableSet &vars, bool free) const { return; }
...
unsigned AtomicFormula::complexity() const { return 0; }

unsigned UnaryConjective::complexity() const { return _op->complexity() + 1; }

unsigned BinaryConjective::complexity() const {
  return _op1->complexity() + _op2->complexity() + 1;
}

unsigned Quantifier::complexity() const { return _op->complexity() + 1; }

// ---------------------------------------------------------------------

// Funkcije za stampanje -----------------------------------------------

void VariableTerm::printTerm(ostream &ostr) const { ostr << _v; }
...
void True::printFormula(ostream &ostr) const { ostr << "True"; }

void False::printFormula(ostream &ostr) const { ostr << "False"; }
...
void Not::printFormula(ostream &ostr) const {
  ...
  if (op_type == T_AND || op_type == T_OR || op_type == T_IMP ||
      op_type == T_IFF)
    ostr << "(";
...
void And::printFormula(ostream &ostr) const {
  ...
  if (op1_type == T_OR || op1_type == T_IMP || op1_type == T_IFF)
    ostr << "(";
  ...
  if (op2_type == T_OR || op2_type == T_IMP || op2_type == T_IFF ||
      op2_type == T_AND)
    ostr << "(";
...
void Or::printFormula(ostream &ostr) const {
  ...
  if (op2_type == T_IMP || op2_type == T_IFF || op2_type == T_OR)
    ostr << "(";
...
Formula AtomicFormula::simplify() { return shared_from_this(); }
...
Formula AtomicFormula::nnf() { return shared_from_this(); }
...
Formula Not::nnf() {
  ...
  /* Primena pravila ~(A<=>B) === (A /\ ~B) \/ (B /\ ~A) */
    ...
    return make_shared<Or>(
        make_shared<And>(iff_op->getOperand1()->nnf(),
...
Formula Iff::nnf() {
  ...
  return make_shared<And>(
      make_shared<Or>(make_shared<Not>(_op1)->nnf(), _op2->nnf()),
      make_shared<Or>(make_shared<Not>(_op2)->nnf(), _op1->nnf()));
}

Formula Forall::nnf() { return make_shared<Forall>(_v, _op->nnf()); }

Formula Exists::nnf() { return make_shared<Exists>(_v, _op->nnf()); }
...
LiteralListList Imp::listCNF() { throw "CNF not aplicable"; }

LiteralListList Iff::listCNF() { throw "CNF not aplicable"; }
...
Formula And::pullquants() {
  ...
  if (_op1->getType() == T_FORALL && _op2->getType() == T_FORALL) {
    ...
    if (fop1->getVariable() == fop2->getVariable()) {
      return make_shared<Forall>(
          fop1->getVariable(),
          make_shared<And>(fop1->getOperand(), fop2->getOperand())
              ->pullquants());
    }
    /* U suprotnom, uvodimo novu promenljivu koja se ne pojavljuje
       ni u jednoj od  formula A i B */
    else {
      Variable var = getUniqueVariable(fop1->getOperand(), fop2->getOperand());
  ...
  /* Slucaj ((exists x) A) /\ B */
    ...
    /* u suprotnom, moramo da preimenujemo vezanu varijablu */
      ...
      return make_shared<Exists>(
          var,
          make_shared<And>(
              eop1->getOperand()->substitute(eop1->getVariable(),
                                             make_shared<VariableTerm>(var)),
              _op2)
              ->pullquants());
    }
...
Formula Or::pullquants() {
  ...
  if (_op1->getType() == T_EXISTS && _op2->getType() == T_EXISTS) {
    ...
    if (fop1->getVariable() == fop2->getVariable()) {
      return make_shared<Exists>(
          fop1->getVariable(),
          make_shared<Or>(fop1->getOperand(), fop2->getOperand())
              ->pullquants());
    }
    /* U suprotnom, uvodimo novu promenljivu koja se ne pojavljuje
       ni u jednoj od  formula A i B */
...
bool unify(const AtomPairs &pairs, Substitution &sub) {
  ...
  for (auto i = pairs.begin(); i != pairs.end(); i++) {
    if (((Atom *)(*i).first.get())->getSymbol() ==
        ((Atom *)(*i).second.get())->getSymbol()) {
...
bool do_unify(TermPairs &pairs) {
  ...
  do {
    ...
    repeat = applyOrientation(pairs) || applyDecomposition(pairs, collision) ||
  ...
  for (auto i = pairs.begin(); i != pairs.end(); i++) {
    ...
    while (j != pairs.end()) {
      if ((*j).first->equalTo((*i).first) &&
        ...
        if (debugFlag)
          cout << "Factoring applied: (" << (*j).first << "," << (*j).second
               << ")" << endl;
        auto erase_it = j;
        j++;
...
void applyTautology(TermPairs &pairs) {
  ...
  while (i != pairs.end()) {
    if ((*i).first->equalTo((*i).second)) {
      if (debugFlag)
        cout << "Tautology applied: (" << (*i).first << "," << (*i).second
             << ")" << endl;
      auto erase_it = i;
      i++;
      pairs.erase(erase_it);
...
bool applyOrientation(TermPairs &pairs) {
  ...
  for (auto i = pairs.begin(); i != pairs.end(); i++) {
    if ((*i).first->getType() != BaseTerm::TT_VARIABLE &&
      ...
      if (debugFlag)
        cout << "Orientation applied: (" << (*i).first << "," << (*i).second
             << ")" << endl;
  ...
  while (i != pairs.end()) {
    if ((*i).first->getType() == BaseTerm::TT_FUNCTION &&
      ...
      if (ff->getSymbol() == fs->getSymbol()) {
        ...
        if (debugFlag)
          cout << "Decomposition applied: (" << (*i).first << "," << (*i).second
               << ")" << endl;
        auto erase_it = i;
        i++;
        pairs.erase(erase_it);
        ...
        if (debugFlag)
          cout << "Collision detected: " << ff->getSymbol()
               << " != " << fs->getSymbol() << endl;
        collision = true;
        return true;
      }
  ...
  for (auto i = pairs.begin(); i != pairs.end(); i++) {
    if ((*i).first->getType() == BaseTerm::TT_VARIABLE) {
      ...
      if ((*i).second->containsVariable(vt->getVariable())) {
        cycle = true;
        if (debugFlag)
          cout << "Cycle detected: " << (*i).second << " contains "
               << vt->getVariable() << endl;
        return true;
      } else {
        bool changed = false;
        for (auto j = pairs.begin(); j != pairs.end(); j++) {
          if (j != i) {
            if ((*j).first->containsVariable(vt->getVariable())) {
              (*j).first =
                  (*j).first->substitute(vt->getVariable(), (*i).second);
              ret = true;
              changed = true;
            }
            if ((*j).second->containsVariable(vt->getVariable())) {
              (*j).second =
                  (*j).second->substitute(vt->getVariable(), (*i).second);
              ret = true;
              changed = true;
            }
          }
        ...
        if (changed) {
          if (debugFlag) {
            cout << "Application applied: (" << (*i).first << "," << (*i).second
                 << ")" << endl;
            cout << pairs << endl;
          }
        }
...
Formula oppositeLiteral(const Formula &l) {
  ...
  if (l->getType() == BaseFormula::T_EQUAL)
    return make_shared<Disequality>(((Equality *)l.get())->getSignature(),
                                    ((Equality *)l.get())->getLeftOperand(),
                                    ((Equality *)l.get())->getRightOperand());
  if (l->getType() == BaseFormula::T_DISEQUAL)
    return make_shared<Equality>(((Disequality *)l.get())->getSignature(),
                                 ((Disequality *)l.get())->getLeftOperand(),
                                 ((Disequality *)l.get())->getRightOperand());
  else
    ...
    for (unsigned j = 0; j < c2.size(); j++) {
      if (c1[i]->getType() == BaseFormula::T_ATOM &&
        ...
        if (c1[i]->getType() == BaseFormula::T_EQUAL) {
          ...
          reverse =
              make_shared<Equality>(tmp->getSignature(), tmp->getRightOperand(),
                                    tmp->getLeftOperand());
        } else {
          Disequality *tmp = (Disequality *)c1[i].get();
          reverse = make_shared<Disequality>(tmp->getSignature(),
                                             tmp->getRightOperand(),
                                             tmp->getLeftOperand());
        }

        if (unify(c1[i], oppositeLiteral(c2[j]), sub) ||
            unify(reverse, oppositeLiteral(c2[j]), sub)) {
          Clause r;
          resolveClauses(c1, c2, i, j, sub, r);
          if (!clauseExists(cnf, r) && !clauseTautology(r)) {
            cnf.push_back(r);
        ...
        if (c2[j]->getType() == BaseFormula::T_NOT) {
          ...
          for (unsigned ii = 0; ii < c2n.size(); ++ii) {
            if (unify(t, c2n[ii], sub)) {
              c2n[ii] = s;
              Formula tmpAtom =
                  make_shared<Atom>(((Atom *)tmp.get())->getSignature(),
                                    ((Atom *)tmp.get())->getSymbol(), c2n);
              Clause tmpClause = c2;
          ...
          for (unsigned ii = 0; ii < c2n.size(); ++ii) {
            if (unify(t, c2n[ii], sub)) {
              c2n[ii] = s;
              Formula tmpAtom =
                  make_shared<Atom>(((Atom *)c2[j].get())->getSignature(),
                                    ((Atom *)c2[j].get())->getSymbol(), c2n);
              Clause tmpClause = c2;
              tmpClause[j] = tmpAtom;
          ...
          for (unsigned ii = 0; ii < c2n.size(); ++ii) {
            if (unify(t, c2n[ii], sub)) {
              c2n[ii] = s;
              Formula tmpAtom = make_shared<Equality>(
                  ((Atom *)c2[j].get())->getSignature(), c2n[0], c2n[1]);
              Clause tmpClause = c2;
              tmpClause[j] = tmpAtom;
          ...
          for (unsigned ii = 0; ii < c2n.size(); ++ii) {
            if (unify(t, c2n[ii], sub)) {
              c2n[ii] = s;
              Formula tmpAtom = make_shared<Disequality>(
                  ((Atom *)c2[j].get())->getSignature(), c2n[0], c2n[1]);
              Clause tmpClause = c2;
              tmpClause[j] = tmpAtom;
        ...
        if (c1[i]->getType() == BaseFormula::T_NOT) {
          ...
          for (unsigned ii = 0; ii < c1n.size(); ++ii) {
            ...
            if (unify(t, c1n[ii], sub)) {
              c1n[ii] = s;
              Formula tmpAtom =
                  make_shared<Atom>(((Atom *)c1[i].get())->getSignature(),
                                    ((Atom *)c1[i].get())->getSymbol(), c1n);
              Clause tmpClause = c1;
              tmpClause[i] = tmpAtom;
          ...
          for (unsigned ii = 0; ii < c1n.size(); ++ii) {
            if (unify(t, c1n[ii], sub)) {
              c1n[ii] = s;
              Formula tmpAtom = make_shared<Equality>(
                  ((Atom *)c1[i].get())->getSignature(), c1n[0], c1n[1]);
              Clause tmpClause = c1;
              tmpClause[i] = tmpAtom;
...
BaseTerm::Type VariableTerm::getType() const { return TT_VARIABLE; }

const Variable &VariableTerm::getVariable() const { return _v; }
...
BaseTerm::Type FunctionTerm::getType() const { return TT_FUNCTION; }

const Signature &FunctionTerm::getSignature() const { return _sig; }

const FunctionSymbol &FunctionTerm::getSymbol() const { return _f; }

const vector<Term> &FunctionTerm::getOperands() const { return _ops; }
...
BaseFormula::Type True::getType() const { return T_TRUE; }
// ----------------------------------------------------------------------

// Klasa False ----------------------------------------------------------

BaseFormula::Type False::getType() const { return T_FALSE; }

// ----------------------------------------------------------------------

// Klasa Atom -----------------------------------------------------------

Atom::Atom(const Signature &s, const PredicateSymbol &p,
           const vector<Term> &ops)
...
const PredicateSymbol &Atom::getSymbol() const { return _p; }

const Signature &Atom::getSignature() const { return _sig; }

const vector<Term> &Atom::getOperands() const { return _ops; }

BaseFormula::Type Atom::getType() const { return T_ATOM; }
...
const Term &Equality::getLeftOperand() const { return _ops[0]; }

const Term &Equality::getRightOperand() const { return _ops[1]; }

BaseFormula::Type Equality::getType() const { return T_EQUAL; }
...
const Term &Disequality::getLeftOperand() const { return _ops[0]; }

const Term &Disequality::getRightOperand() const { return _ops[1]; }

BaseFormula::Type Disequality::getType() const { return T_DISEQUAL; }
...
UnaryConjective::UnaryConjective(const Formula &op) : _op(op) {}

const Formula &UnaryConjective::getOperand() const { return _op; }

// -----------------------------------------------------------------------

// Klasa Not -------------------------------------------------------------

BaseFormula::Type Not::getType() const { return T_NOT; }
...
const Formula &BinaryConjective::getOperand1() const { return _op1; }

const Formula &BinaryConjective::getOperand2() const { return _op2; }

// Klasa And ---------------------------------------------------------------

BaseFormula::Type And::getType() const { return T_AND; }

// -------------------------------------------------------------------------

// Klasa Or ----------------------------------------------------------------

BaseFormula::Type Or::getType() const { return T_OR; }

// -------------------------------------------------------------------------

// Klasa Imp ---------------------------------------------------------------

BaseFormula::Type Imp::getType() const { return T_IMP; }

// -------------------------------------------------------------------------

// Klasa Iff ---------------------------------------------------------------

BaseFormula::Type Iff::getType() const { return T_IFF; }

// -------------------------------------------------------------------------

// Klasa Quantifier --------------------------------------------------------

Quantifier::Quantifier(const Variable &v, const Formula &op) : _v(v), _op(op) {}

const Variable &Quantifier::getVariable() const { return _v; }

const Formula &Quantifier::getOperand() const { return _op; }

// ------------------------------------------------------------------------

// Klasa Forall -----------------------------------------------------------

BaseFormula::Type Forall::getType() const { return T_FORALL; }

// ------------------------------------------------------------------------

// Klasa Exists -----------------------------------------------------------

BaseFormula::Type Exists::getType() const { return T_EXISTS; }
